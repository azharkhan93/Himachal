$(document).ready(function () {
  ...
  $("#staticContactForm input, #staticContactForm textarea").on(
    ...
    function () {
      setTimeout(function () {
        ...
        if (!$("#staticContactForm input:focus, #staticContactForm textarea:focus").length) {
  ...
  $(window).scroll(function () {
    ...
    const isFormFocused = $("#staticContactForm input:focus, #staticContactForm textarea:focus").length > 0;
    const shouldKeepVisible = isFormBeingUsed || isFormFocused;
    const isMobile = window.innerWidth < 768;
    
    // Mobile: Hide when navbar overlaps form (with delay)
    if (isMobile) {
      const navbarOverlaps = (scrollTop + $("#navbar").outerHeight()) >= heroForm.offset().top;
